/* MDA Manutenção — interações sem dependências externas */
(() => {
  ...
  const form = document.querySelector('#lead-manutencao-form');
  if (!form) return;
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const message = encodeURIComponent(['Olá, quero um orçamento para um reparo/serviço.', `Nome: ${data.name}`, `Tipo de imóvel: ${data.profile}`, `Cidade: ${data.city}`, `Necessidade: ${data.need}`, `Prazo: ${data.timing}`, `Canal: ${data.preferred_channel}`, data.context ? `Descrição: ${data.context}` : '', data.email ? `E-mail: ${data.email}` : ''].filter(Boolean).join(String.fromCharCode(10)));
      const whatsappLink = document.createElement('a');
      whatsappLink.className = 'btn btn-ghost lead-whatsapp-link';
      whatsappLink.href = `https://wa.me/5562992914190?text=${message}`;
      whatsappLink.target = '_blank';
      whatsappLink.rel = 'noopener';
      whatsappLink.textContent = 'Enviar também pelo WhatsApp (opcional) →';
      status.replaceChildren(document.createTextNode('Solicitação registrada. Se preferir, você pode encaminhar uma cópia pelo WhatsApp: '), whatsappLink);
      status.className = 'form-status success'; form.reset(); showStep(1);
    } catch (error) {
      status.textContent = 'Não foi possível enviar agora. Fale diretamente pelo WhatsApp ou tente novamente.'; status.className = 'form-status error';
    } finally {
