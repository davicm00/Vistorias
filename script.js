...
(() => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const message = encodeURIComponent(['Olá, quero falar sobre uma avaliação técnica.', `Nome: ${data.name}`, `Perfil: ${data.profile}`, `Cidade: ${data.city}`, `Necessidade: ${data.need}`, `Prazo: ${data.timing}`, `Canal: ${data.preferred_channel}`, data.context ? `Contexto: ${data.context}` : '', data.email ? `E-mail: ${data.email}` : ''].filter(Boolean).join('\\n'));
      window.open(`https://wa.me/5562992914190?text=${message}`, '_blank', 'noopener');
      status.textContent = 'Solicitação preparada. A equipe poderá continuar o atendimento pelo WhatsApp.'; status.className = 'form-status success'; form.reset(); showStep(1);
